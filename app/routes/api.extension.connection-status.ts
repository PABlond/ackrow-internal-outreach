...
export async function action({ request }: Route.ActionArgs) {
  ...
  if (!id) {
    return data({ ok: false, error: "Missing prospect id" }, { status: 400, headers: corsHeaders() });
  }

  if (payload.state === "unknown") {
    return data({ ok: true, synced: false }, { headers: corsHeaders() });
  }

  if (!["accepted", "declined", "pending"].includes(String(payload.state))) {
    return data({ ok: false, error: "Unknown connection state" }, { status: 400, headers: corsHeaders() });
  }

  await syncProspectConnectionState(id, payload.state as "accepted" | "declined" | "pending");
  return data({ ok: true, synced: true, state: payload.state }, { headers: corsHeaders() });
}
